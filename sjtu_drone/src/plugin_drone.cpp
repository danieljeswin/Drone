...
void DroneSimpleController::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("botName"))
    bot_name_.clear();
  else
    bot_name_ = _sdf->GetElement("botName")->Get<std::string>();
  ...
  //load parameters
  cmd_normal_topic_ = bot_name_ + "/cmd_vel";
  takeoff_topic_ = bot_name_ + "/takeoff";
  land_topic_ = bot_name_ + "/land";
  reset_topic_ = bot_name_ + "/reset";
  posctrl_topic_ = bot_name_ + "/posctrl";
  gt_topic_ = bot_name_ + "/gt_pose";
  switch_mode_topic_ = bot_name_ + "/vel_mode";
  ...
  if (!gt_topic_.empty()){
      pub_gt_pose_ = node_handle_->advertise<geometry_msgs::Pose>(bot_name_ + "/gt_pose",1024);    
  }
  
  pub_gt_vec_ = node_handle_->advertise<geometry_msgs::Twist>(bot_name_ + "/gt_vel", 1024);
  pub_gt_acc_ = node_handle_->advertise<geometry_msgs::Twist>(bot_name_ + "/gt_acc", 1024);
